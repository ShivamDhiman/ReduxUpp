...
const DependencyClauseCard = (props) => {
  const { styles, setDependencyData, formType, showErrors, formDependencyMapping, currentFormData } = props;
    ...
    switch (name) {
      ...
      case 'dependent_form_code':
        ...
        dependencyClause[index] = {
          ...
          loading: true,
          variable_name: '',
          form_group: selectedForm?.form_group,
          // operator: '',
          values: '',
          question: { ...dummyQuestion },
        };
        ...
        if (response.data && response.data.success === true && response.data.data) {
          ...
          if (response.data.data?.length) {
            ...
            const filterData = formData.filter((item) => {
              return item.name !== currentFormData.name && item.version !== currentFormData.verSion;
            });
            setDependencyForms([...filterData]);
          }
        }
      })
      .catch((error) => {
