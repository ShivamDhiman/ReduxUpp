...
const FormAddEditComponent = (props) => {
  ...
  const [formSettingDataState, setFormSettingsDataState] = useState('');
  const eCIFlist = useSelector((state) => state.eicf.ecifPublished);
  const { name, version, description, editStatus, event_name } = state;
  ...
  const addUpdateForm = (isPublishing) => {
    ...
    const {
      ...
      form_expire_time,
      participant_facing,
      email_version = state?.email_version,
      email_reminder_version = state?.email_reminder_version,
      email_code = state?.email_code,
      email_reminder_code = state?.email_reminder_code,
      days_reminder,
      days,
      hours_reminder,
      hours,
    ...
    const constructFormData = (item) => {
      return {
        ...
        form_expire_time,
        participant_facing,
        email_code: email_code || state?.email_code,
        email_version: email_version || state?.email_version,
        email_reminder_code: email_reminder_code || state?.email_reminder_code,
        email_reminder_version: email_reminder_version || state?.email_reminder_version,
        days_reminder: days_reminder,
        days: days,
        hours_reminder: hours_reminder,
        hours: hours,
    ...
    for (const property in payload[0]) {
      switch (property) {
        ...
        case 'email_code':
        case 'email_version':
          if (participant_facing && (Validation.empty(email_code) || Validation.empty(email_version))) {
            return;
          }
          break;
        case 'email_reminder_code':
        case 'email_reminder_version':
          if (
            participant_facing &&
            (days_reminder || hours_reminder) &&
            (Validation.empty(email_reminder_code) || Validation.empty(email_reminder_version))
          ) {
            return;
          }
          break;
        case 'form_expire_time':
          if (form_expire && Validation.empty(form_expire_time)) return;
          break;
        case 'days_reminder':
          if (days_reminder && Validation.empty(days)) return;
          break;
        case 'hours_reminder':
          if (hours_reminder && Validation.empty(hours)) return;
          break;
        case 'event_name':
          if (Validation.empty(event_name)) return;
          break;
        default:
          break;
      }
    ...
    //   // now validating all language payload
    //   payload.map((langPayload, langPayloadIndex) => {
    //     langPayload.FormsSections = langPayload?.FormsSections.map((entry, i) => {
    //       delete entry.expand;
    //       entry.order = i + 1;
    //       entry.language = 'English';
    //       let hasICFNameBlank = [
    //         Validation.empty(langPayload.eICFName1),
    //         Validation.empty(langPayload.eICFName2),
    //         Validation.empty(langPayload.eICFName3),
    //       ].every((icf) => icf);
    //       if (
    ...
    /* use in english spanish older one*/
      ...
      .then((response) => {
        if (response.data && response.data.success === true && response.data.data) {
          ...
          if (editInfo?.formCategory == 'Use Variables from existing form') {
            ...
            if (listEnglish) {
              listEnglish.form_code = '';
            }
            if (listSpanish) {
              listSpanish.form_code = '';
            }
          }
          ...
          if (!editInfo?.formCategory || editInfo?.formCategory !== 'Use Variables from existing form') {
            ...
            setState({
              ...state,
              days_reminder: listEnglish?.days_reminder,
              event_name: listEnglish?.event_name,
              hours_reminder: listEnglish?.hours_reminder,
              days: listEnglish?.days,
              email_code: listEnglish?.email_code,
              email_version: listEnglish?.email_version,
              email_reminder_code: listEnglish?.email_reminder_code,
              email_reminder_version: listEnglish?.email_reminder_version,
              hours: listEnglish?.hours,
  ...
  useEffect(() => {
    ...
    const formType = decodeData(router.query.type) || decodeData(router.query.id);
    const category = formType?.formType || formType?.category;
    const encryptdata = encodeData({ status, category });
    dispatch(setEicformsPublished(encryptdata));
  ...
  const previewAction = () => {
    ...
    setFormSettingsDataState(formSettingsData);
    const {
      reminder,
      form_expire,
      form_expire_time,
  ...
  const setPreviewData = (language) => {
    ...
    content.FormsSections.map((section) => {
  ...
  return (
    <div className={`${styles.formContainer}`}>
      ...
      <div className="row">
        ...
        {/* it will be work in last multilanguage version */}
      ...
      {!preview ? (
        <>
          <div className="row">
            ...
            <div className="col-md-3">
              <Form.Group className="col" controlId="formEventName">
                ...
                <InputGroup>
                  <Form.Control
                    ...
                    onChange={handleChange}
                    value={event_name}
                    isInvalid={showErrors && Validation.empty(event_name)}
                    required
                  />
                  <Form.Control.Feedback type="invalid">
          ...
          <FormComponent
            ...
            formType={formType}
            currentFormData={paramData}
            formSettingDataState={formSettingDataState}
          />
        </>
      ) : (
        <>
          ...
          <div className=" col md-6 d-flex justify-content-end mb-2 ">
            {languageListPicked && languageListPicked.length
              ? languageListPicked.map((langItem, index) => (
                  <button
                    ...
                    name={langItem}
                    className={`btn px-4 d-inline-block text-white me-1 ${
                      selectedPreviewContent?.language === langItem ? styles.button : 'bg-gray'
                    } ${styles.outlineNone}`}
                  >
                    {langItem}
                  </button>
            ...
            {/* <button
              onClick={selectLanguage}
              name="English"
              className={`btn px-4 d-inline-block text-white ${selectedPreviewContent?.language === 'English' ? styles.button : 'bg-gray'
                } ${styles.outlineNone}`}
            >
              English
            </button>
            <button
              onClick={selectLanguage}
              name="Spanish"
              className={`btn px-4 d-inline-block text-white ms-1  me-4  ${selectedPreviewContent?.language === 'Spanish' ? styles.button : 'bg-gray'
                } ${styles.outlineNone}`}
            >
              Spanish
            </button> */}
